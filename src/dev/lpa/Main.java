...
public class Main {
    public static void main(String[] args) {
        ...
        Card.printDeck(cards, "Shuffled", 4);

        List<Card> playerOneHand = new ArrayList<>(5);
        List<Card> playerTwoHand = new ArrayList<>(5);
        List<Card> playerThreeHand = new ArrayList<>(5);


        List <List<Card>> players = new ArrayList<>(List.of(playerOneHand, playerTwoHand, playerThreeHand));

        for (int i = 0; i<5; i++){
            for (var player : players){
                player.add(cards.get(cards.size() -1));
                cards.remove(cards.size() -1);
            }
        }

        Card.printDeck(playerOneHand, "p1:", 1);
        Card.printDeck(playerTwoHand, "p2", 1);
        Card.printDeck(playerThreeHand, "p3", 1);
